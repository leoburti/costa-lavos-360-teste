...
const RankingCategory = ({ data }) => {
  const totalRevenue = useMemo(() => data?.reduce((sum, item) => sum + item.value, 0) || 0, [data]);
  const maxRevenue = useMemo(() => data?.length > 0 ? data.reduce((max, item) => item.value > max ? item.value : max, 0) : 0, [data]);
  ...
  return (
    <ScrollArea className="h-[450px]">
        <div className="flex items-center py-2 px-4 bg-muted/50 font-semibold text-sm text-muted-foreground">
          <div className="w-8 text-center">#</div>
          <div className="flex-1 min-w-0 mx-4">Item</div>
          <div className="w-28 text-right shrink-0">Qtde. Vendida</div>
          <div className="w-40 text-right shrink-0">Receita</div>
          <div className="w-24 text-right shrink-0 ml-4">Tendência</div>
      </div>
      <div>
        {processedData.map((item, index) => (
          <RankingListItem key={item.name} item={item} index={index} maxRevenue={maxRevenue} />
        ))}
      </div>
    </ScrollArea>
  );

    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-full overflow-hidden ring-1 ring-slate-200">
      <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Matriz de performance por supervisor</p>
        </div>
        <Badge variant="outline" className="bg-white text-slate-500 text-[10px] font-normal">
          {data?.length || 0} Supervisores
        </Badge>
      </div>
                ...
                data.map((item, index) => {
                  ...
                  return (
                    <TableRow 
                      ...
                      onClick={() => onSelectSupervisor && onSelectSupervisor(item.name)}
                    >

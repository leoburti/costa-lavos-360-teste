...
export default function AnaliticoGrupoClientesPage() {
  ...
  const CustomizedContent = (props) => {
    ...
    return (
      <g>
        <rect x={x} y={y} width={width} height={height} style={{ fill: '#f59e0b', stroke: '#fff', strokeWidth: 2, opacity: 0.9 }} />
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="text-muted-foreground">Desempenho de vendas por grupo de clientes.</p>
        </div>

        {loading ? (
          <LoadingState message="Analisando dados dos grupos..." />
        ) : error ? (
          <ErrorState error={error} onRetry={retry} />
        ) : !data || data.length === 0 ? (
          <EmptyState description="Nenhuma venda encontrada para os grupos de clientes no período selecionado." />
        ) : (
          <Card className="border-slate-200 shadow-sm">
            <CardHeader>
              <CardTitle>Faturamento por Grupo de Clientes</CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="mt-10">
                <h3 className="text-lg font-semibold mb-4 text-slate-800">Detalhamento por Grupo</h3>

...
export default function RelatoriVendasPorCliente() {
  ...
  const { data, loading, error, retry } = useAnalyticalData(
    'get_relatorio_vendas_por_cliente',
    params,
    {
      onError: (err) => {
        toast({
  ...
  const kpis = useMemo(() => {
    ...
    return [
      { label: 'Total de Vendas', value: `R$ ${totalSales.toLocaleString('pt-BR')}`, change: 11.2 },
      { label: 'Total de Clientes', value: data.length, change: 7.5 },
      { label: 'Melhor Cliente', value: topClient?.client_name || '-', change: 22.1 },
      { label: 'Venda Média', value: `R$ ${(totalSales / data.length).toLocaleString('pt-BR')}`, change: 4.8 },
    ];
  }, [data]);
